...
//   const Joi = require('joi');
  ...
  const outpassValidation = (req, res, next) => {
      ...
      const schema = Joi.object({
          ...
          registrationNumber: Joi.string().length(5).length(6).pattern(/^\d{5,6}$/),
          rollNumber: Joi.string().length(4).pattern(/^\d{4}$/),
          reason: Joi.string().min(10),
          date: Joi.date().iso(),
          startHour: Joi.string().pattern(/^([01]\d|2[0-3]):([0-5]\d)$/),
          endHour: Joi.string().pattern(/^([01]\d|2[0-3]):([0-5]\d)$/),
          contactNumber: Joi.string().length(10).pattern(/^\d{10}$/),
          ...
          extraDataArray: Joi.array().items(Joi.number()).length(4),
          extraValidation: Joi.array().items(Joi.number()).length(2),
          outTime: Joi.string().pattern(/^([01]\d|2[0-3]):([0-5]\d)$/),
          inTime: Joi.string().pattern(/^([01]\d|2[0-3]):([0-5]\d)$/)
      });
...
//   module.exports = { outpassValidation };
const leaveValidation = (req, res, next) => {
    ...
    const schema = Joi.object({
        ...
        // startHour: Joi.string().pattern(/^([01]\d|2[0-3]):([0-5]\d)$/).required(), // Validates time in HH:MM format
        // endHour: Joi.string().pattern(/^([01]\d|2[0-3]):([0-5]\d)$/).required(), // Validates time in HH:MM format
        ...
        extraValidation: Joi.array().items(Joi.number()).length(2).default([0, 0]), // Default value of [0, 0] and validation
        outTime: Joi.string().pattern(/^([01]\d|2[0-3]):([0-5]\d)$/).allow(''), // Validates time in HH:MM format, default to empty string if not provided
        inTime: Joi.string().pattern(/^([01]\d|2[0-3]):([0-5]\d)$/).allow('') // Validates time in HH:MM format, default to empty string if not provided
    });
